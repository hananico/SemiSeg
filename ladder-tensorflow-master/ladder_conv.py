...
layer_sizes = [28*28, 1000, 500, 250, 250, 250, 256*256]
...
rgb, labels = read_data.get_input('train_quad.txt', batch_size)

print(rgb)
print(labels)
# there is no ops to read matlab file, load it independently from queue runner.
LABEL_CACHE = read_data.CACHE_LABEL('train_quad.txt', 32)
print(LABEL_CACHE)
...
for i in tqdm(range(i_iter, num_iter)):
    ...
    with open('train_log', 'ab') as train_log:
            ...
            log_i =  sess.run([accuracy], feed_dict={inputs: mnist.test.images, outputs:mnist.test.labels, training: False})
